...
1.  **Density estimation** — given wide-bin counts that are a
    multinomial sample of an underlying continuous variable, recover the
    smooth latent density (Lambert and Eilers, 2009).
...
- [`pclm()`](https://christk.github.io/pclmbayes/reference/pclm.md) /
  ...
  frequentist (BIC) and Bayesian fits of the Lambert–Eilers density
  model, with optional shape constraints (`unimodal`, `logconcave`,
  `monotonic`).
- [`pclm_exact()`](https://christk.github.io/pclmbayes/reference/pclm_exact.md)
  — constrained MAP that gives the smoothest density consistent with the
  wide-bin counts *exactly*.
- [`calibrate()`](https://christk.github.io/pclmbayes/reference/calibrate.md)
  ...
  constraint manifold; for Bayesian fits, every posterior draw is
  calibrated.
- [`posterior_predict()`](https://christk.github.io/pclmbayes/reference/posterior_predict.md)
  — per-cell credible/prediction intervals for the ungrouped fine-cell
  counts, with multinomial sampling noise.
...
argument,
e.g. `remotes::install_github("ChristK/pclmbayes", ref = "main")`.
...
with the identifiability constraint $`\sum_k \phi_k = 0`$. An $`r`$-th
order discrete difference penalty on the spline coefficients $`\phi`$
provides smoothing. The wide-bin probabilities $`\gamma = C\pi`$ are
linked to the observed counts $`m`$ through a multinomial likelihood.
The Bayesian variant places a Gaussian prior on $`\Delta^r\phi`$ with
...
- `bloodlead` is the verbatim seven-bin frequency table from Hasselblad,
  Stead and Galke (1980) reproduced in Lambert and Eilers (2009, Section
  6.1). The “65+” bin is bounded above by 80 µg/dl as in the paper.
- `tbdeaths1907` is an **illustrative reconstruction** of the dataset
  ...
  deaths and a plausible early-20th-century age-mortality shape, and is
  intended for demonstration of the methodology rather than for
...
- Lambert, P. and Eilers, P. H. C. (2009). Bayesian density estimation
  ...
  <doi:%5B10.1016/j.csda.2008.11.022>\](<https://doi.org/10.1016/j.csda.2008.11.022>)
